use crate::library::base_lib_key::{
    CLOUDINARY_API_BASE_URL, CLOUDINARY_API_CLOUD_NAME, CLOUDINARY_API_KEY, CLOUDINARY_API_SECRET,
};
...
pub const UPLOAD_PRESET: &[&str] = &["profile_pictures"];

pub async fn cloudinary_upload_image_profile(
    form: UploadForm<'_>,
    user: AuthenticatedUser,
) -> Result<Json<CloudinaryResponse>, String> {
    ...
    let url_api = format!(
        "{}/{}/image/upload",
        *CLOUDINARY_API_BASE_URL, *CLOUDINARY_API_CLOUD_NAME
    );
    ...
    let response = client
        ...
        .basic_auth(
            ...
            Some(CLOUDINARY_API_SECRET.to_string()),
        )
        .send()
        .await
    ...
    if status == 200 {
        ...
        Err(format!("Upload failed with status {status}: {cld_error}"))
    }
}
