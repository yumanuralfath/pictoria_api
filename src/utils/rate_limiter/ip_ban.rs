...
use crate::library::base_lib_key::{WEBHOOK_IP_BAN_PASSWORD, WEBHOOK_IP_BAN_URL, WEBHOOK_IP_BAN_USER};
...
pub async fn send_block_webhook(ip: IpAddr, payload: Option<Value>) {
    ...
    let res = client
        .post(url)
        .basic_auth(WEBHOOK_IP_BAN_USER.to_string(), Some(WEBHOOK_IP_BAN_PASSWORD.to_string()))
        .json(&body)
        .send()
        .await;
