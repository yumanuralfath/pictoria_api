...
pub fn search_context_from_json(user_prompt: &str) -> String {
    let items: Vec<ContextItem> = serde_json::from_str(CONTEXT_JSON)
        .expect("Format JSON tidak valid");
    ...
    for item in &items {
        ...
        let keyword_match = item.keywords.iter().any(|keyword| lower_prompt.contains(keyword));
